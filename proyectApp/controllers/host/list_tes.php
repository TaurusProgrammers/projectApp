						...
						<div class="row">
							<div class="col-xs-12">
										...
										?>
											...
											<thead>
												<tr>
													<th class="detail-col">Testigo</th>
													<th>Cedula</th>
													<th>usuario</th>
													<th>contraseña</th>
												</tr>
											...
											<tbody>
												<?php
													$año=date ("Y");
													$sqlc=mysqli_query($con, "SELECT * FROM testigo WHERE cedula='".$_POST['cedula']."'");
													while ($res=mysqli_fetch_assoc($sqlc)) {
														$name=$res['name'];
														$last=$res['last_name'];
														$cedula=$res['cedula'];
														$usuario='';
														$sql=mysqli_query($con, "SELECT usuario FROM usuarios WHERE cedula='".$cedula."'LIMIT 1");
														if($sq=mysqli_fetch_assoc($sql)){
															$usuario=$sq['usuario'];
														}
										...
										<?php }else{
											...
											<tbody>
												<?php
													$año=date ("Y");
													$sqlc=mysqli_query($con, "SELECT * FROM testigo");
													while ($res=mysqli_fetch_assoc($sqlc)) {
														$name=$res['name'];
														$last=$res['last_name'];
														$cedula=$res['cedula'];

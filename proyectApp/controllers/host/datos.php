								...
								<?php if(isset($_POST['cedula'])){
									...
									<table id="simple-table" class="table  table-bordered table-hover">
											...
											<tbody>
												...
												if($_GET['car']==1){
													$cont=0;
													$sql=mysqli_query($con, "SELECT nombres, apellidos, cedula, lider, confirm, celular, municipio FROM usuarios WHERE cedula='".$_POST['cedula']."' and  senado='".$_GET['cand']."' and id_rol='0'");
													while($sr=mysqli_fetch_assoc($sql)){
														$cont++;
														$lider='';
														$sqlx=mysqli_query($con, "SELECT * FROM lideres WHERE cedula='".$sr['lider']."' LIMIT 1");
														if($srs=mysqli_fetch_assoc($sqlx)){
															$lider=$srs['name'].' '.$srs['last_name'];
														}
														if($sr['confirm']==1){
														...
														echo '<td><b>'.$lider.'</b></td>';
														echo $checked;
														echo '</tr>';
													}
												}elseif($_GET['car']==2){
													$cont=0;
													$sql=mysqli_query($con, "SELECT nombres, apellidos, cedula, lider, confirm, celular, municipio FROM usuarios WHERE cedula='".$_POST['cedula']."' and camara='".$_GET['cand']."' and id_rol='0'");
													while($sr=mysqli_fetch_assoc($sql)){
														$cont++;
														$lider='';
														$sqlx=mysqli_query($con, "SELECT * FROM lideres WHERE cedula='".$sr['lider']."' LIMIT 1");
								...
								<?php }else{
									...
									<table id="simple-table" class="table  table-bordered table-hover">
											...
											<tbody>
												...
												if($_GET['car']==1){
													$cont=0;
													$sql=mysqli_query($con, "SELECT nombres, apellidos, cedula, lider, confirm, celular, municipio FROM usuarios WHERE  senado='".$_GET['cand']."' and id_rol='0'");
													while($sr=mysqli_fetch_assoc($sql)){
														$cont++;
														$lider='';
														$sqlx=mysqli_query($con, "SELECT * FROM lideres WHERE cedula='".$sr['lider']."' LIMIT 1");
													...
													$cont=0;
													$sql=mysqli_query($con, "SELECT nombres, apellidos, cedula, lider, confirm, celular, municipio FROM usuarios WHERE camara='".$_GET['cand']."' and id_rol='0'");
													while($sr=mysqli_fetch_assoc($sql)){
														$cont++;
														$lider='';
														$sqlx=mysqli_query($con, "SELECT * FROM lideres WHERE cedula='".$sr['lider']."' LIMIT 1");
